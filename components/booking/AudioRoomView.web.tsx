/**
 * AudioRoomView.web.tsx
 * Full web audio room powered by livekit-client (browser SDK).
 * Identical layout and UX to the native AudioRoomView:
 *   • Stage — speaker avatar grid with speaking ring animation
 *   • Audience — listener chips with hand-raise indicator
 *   • Controls — mute/unmute, raise hand, leave
 *   • Host hand-raise panel (slides in from right)
 */
...
function getMeta(p: Participant): ParticipantMeta {
  try {
...
function SpeakingRing({ active }: { active: boolean }) {
  ...
  return (
    <Animated.View
      pointerEvents="none"
      style={[styles.speakingRing, { transform: [{ scale }], opacity }]}
    />
  );
}
  ...
  return (
    <View style={styles.speakerCard}>
      ...
      <View style={[styles.speakerAvatar, isSpeaking && !isMuted && styles.speakerAvatarActive]}>
        <Text style={styles.speakerInitial} allowFontScaling={false}>{initial}</Text>
      </View>
      <View style={[styles.speakerMicBadge, isMuted && styles.speakerMicBadgeMuted]}>
        <Ionicons name={isMuted ? "mic-off" : "mic"} size={11} color={isMuted ? Colors.statusDanger : "#FFF"} />
      </View>
...
function ListenerChip({ name, handRaised }: { name: string; handRaised: boolean }) {
  ...
  return (
    <View style={[styles.listenerChip, handRaised && styles.listenerChipHandRaised]}>
      ...
      <Text style={styles.listenerName} numberOfLines={1} allowFontScaling={false}>{name}</Text>
      {handRaised && <Ionicons name="hand-left" size={12} color={Colors.statusWarning} />}
    </View>
  );
}
...
function HandRaisePanel({
  ...
  return (
    <Animated.View style={[styles.handPanel, { transform: [{ translateX: slideAnim }] }]}>
      ...
      {raisedHands.length === 0 ? (
        <View style={styles.handPanelEmpty}>
          <Text style={styles.handPanelEmptyText} allowFontScaling={false}>No raised hands</Text>
        </View>
      ) : (
        <ScrollView showsVerticalScrollIndicator={false}>
          {raisedHands.map((p) => (
            <View key={p.identity} style={styles.handPanelRow}>
              ...
              <Text style={styles.handPanelName} numberOfLines={1} allowFontScaling={false}>{p.name}</Text>
              <TouchableOpacity
              ...
              >
                ...
                <Text style={styles.promoteBtnText} allowFontScaling={false}>Promote</Text>
              </TouchableOpacity>
            </View>
          ))}
        </ScrollView>
...
export function AudioRoomView({
  ...
  const [tick, setTick] = useState(0); // force re-render on participant events
  ...
  useEffect(() => {
    ...
    room.on(RoomEvent.ConnectionStateChanged, (state) => {
      if (state === ConnectionState.Connected) {
        ...
        syncParticipants();
        // Audio-only: enable mic if HOST or SPEAKER
        if (role !== "LISTENER") {
    ...
    room.connect(wsUrl, token).catch(() => setConnecting(false));
  ...
  const localIsSpeaker = localRole === "HOST" || localRole === "SPEAKER";

  const speakers = remoteParticipants.filter((p) => {
    const r = getMeta(p).role;
    return r === "SPEAKER" || r === "HOST";
  });
  const listeners = remoteParticipants.filter((p) => getMeta(p).role === "LISTENER");
  ...
  const handleToggleMic = useCallback(async () => {
    if (!localIsSpeaker || !roomRef.current) return;
  ...
  }, [localIsSpeaker, isMuted, eventId, updateMutedStatus]);
  ...
  const handlePromote = useCallback(async (targetIdentity: string) => {
    if (!eventId) return;
    try {
      await promoteToSpeaker({ eventId: eventId as any, targetUserId: targetIdentity as any });
    } catch (err: any) {
    ...
    Alert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              await demoteToListener({ eventId: eventId as any, targetUserId: targetIdentity as any });
            } catch (err: any) {
          ...
          <View style={styles.speakerGrid}>
            ...
            {!localIsSpeaker && speakers.length === 0 && (
              <View style={styles.emptyStage}>
                ...
                <Text style={styles.emptyStageText} allowFontScaling={false}>No speakers yet</Text>
              </View>
            )}
          </View>
        </View>
            ...
            <>
              ...
              >
                <Ionicons
                  ...
                  size={22}
                  color={isMuted ? Colors.statusDanger : "#FFF"}
                />
              ...
              {isHost && (
                ...
                >
                  <Ionicons
                    ...
                    size={22}
                    color={raisedHands.length > 0 ? Colors.statusWarning : "#FFF"}
                  />
                  {raisedHands.length > 0 && (
            ...
            <>
              ...
              >
                <Ionicons
                  ...
                  size={22}
                  color={handRaised ? Colors.statusWarning : "#FFF"}
                />
                <Text
                  style={[styles.ctrlLabel, handRaised && { color: Colors.statusWarning }]}
...
// ─── Styles (mirrors AudioRoomView.tsx 1:1) ───────────────────────────────────
const styles = StyleSheet.create({
  root: { flex: 1, backgroundColor: "#0D0820", position: "relative" },

  bgLayer1: { ...StyleSheet.absoluteFillObject, backgroundColor: "#1A0A35", opacity: 1 },
  bgLayer2: {
    position: "absolute", top: 0, left: 0, right: 0, height: "60%",
    backgroundColor: "#2D1060", opacity: 0.45,
  },
  bgLayer3: {
    position: "absolute", bottom: 0, left: 0, right: 0, height: "40%",
    backgroundColor: "#080515", opacity: 0.7,
  },

  connectingOverlay: {
    ...StyleSheet.absoluteFillObject,
    alignItems: "center", justifyContent: "center",
    gap: spacing.space3, zIndex: 50,
    backgroundColor: "rgba(10,5,25,0.80)",
  },
  connectingText: { ...typeScale.headingSM, color: "rgba(255,255,255,0.7)" },
  ...
  topLeft: { flexDirection: "row", alignItems: "center", gap: spacing.space2, width: 80 },
  topTitle: { ...typeScale.headingSM, color: "#FFF", fontWeight: "700", flex: 1, textAlign: "center" },
  topRight: {
  ...
  liveBadge: {
    ...
    backgroundColor: "rgba(139,92,246,0.25)", borderRadius: radius.radiusFull,
    paddingHorizontal: 9, paddingVertical: 4,
    borderWidth: 1, borderColor: "rgba(139,92,246,0.50)",
  },
  liveDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: Colors.palette.purple },
  liveBadgeText: { fontSize: 10, fontWeight: "800", color: Colors.palette.purple, letterSpacing: 0.8 },
  participantCount: { flexDirection: "row", alignItems: "center", gap: 4 },
  participantCountText: { fontSize: 13, color: "rgba(255,255,255,0.7)", fontWeight: "600" },
  handBtn: {
  ...
  handBadge: {
    ...
    borderRadius: 7, backgroundColor: Colors.statusWarning,
    alignItems: "center", justifyContent: "center", paddingHorizontal: 3,
  },
  handBadgeText: { fontSize: 8, fontWeight: "700", color: "#000" },

  sectionLabel: {
    fontSize: 10, fontWeight: "700", color: "rgba(255,255,255,0.35)",
    letterSpacing: 1.2, marginBottom: spacing.space3,
    paddingHorizontal: spacing.space4,
  },

  stageSection: { flex: 1, paddingTop: spacing.space5 },
  speakerGrid: {
    flexDirection: "row", flexWrap: "wrap", justifyContent: "center",
    paddingHorizontal: spacing.space4, gap: spacing.space4,
  },
  emptyStage: {
    alignItems: "center", justifyContent: "center",
    gap: spacing.space2, paddingVertical: spacing.space8, width: "100%",
  },
  emptyStageText: { ...typeScale.bodySM, color: "rgba(255,255,255,0.25)" },

  speakerCard: {
    alignItems: "center", gap: 6, width: 80, position: "relative",
  },
  speakerAvatar: {
    width: 64, height: 64, borderRadius: 32,
    backgroundColor: "rgba(139,92,246,0.25)",
    alignItems: "center", justifyContent: "center",
    borderWidth: 2, borderColor: "rgba(139,92,246,0.30)",
  },
  speakerAvatarActive: {
    borderColor: Colors.palette.purple,
    backgroundColor: "rgba(139,92,246,0.35)",
  },
  speakerInitial: { fontSize: 22, fontWeight: "700", color: Colors.palette.purple },
  speakingRing: {
    position: "absolute", top: -4, left: "50%", marginLeft: -36,
    width: 72, height: 72, borderRadius: 36,
    borderWidth: 3, borderColor: Colors.palette.purple,
  },
  speakerMicBadge: {
  ...
  speakerMicBadgeMuted: {
    backgroundColor: Colors.statusDangerBg, borderColor: Colors.statusDanger,
  },
  speakerName: {
    fontSize: 11, fontWeight: "600",
    color: "rgba(255,255,255,0.85)", textAlign: "center", width: 72,
  },
  demoteBtn: { marginTop: 2 },

  stageDivider: {
    height: 1, backgroundColor: "rgba(255,255,255,0.06)",
    marginHorizontal: spacing.space4, marginVertical: spacing.space3,
  },

  audienceSection: { paddingBottom: spacing.space4, maxHeight: 120 },
  audienceScroll: { paddingHorizontal: spacing.space4, gap: spacing.space2 },
  listenerChip: {
  ...
  listenerInitial: { fontSize: 10, fontWeight: "700", color: "rgba(255,255,255,0.7)" },
  listenerName: {
    fontSize: 11, color: "rgba(255,255,255,0.65)", fontWeight: "500", maxWidth: 80,
  },

  controlsBar: {
    flexDirection: "row", alignItems: "center", justifyContent: "center",
    gap: spacing.space6, paddingVertical: spacing.space4,
  ...
  ctrlBtnMuted: { backgroundColor: Colors.statusDangerBg, borderColor: Colors.statusDanger },
  ctrlBtnHandRaised: {
  ...
  ctrlBtnLeave: { backgroundColor: Colors.statusDanger, borderColor: Colors.statusDanger },
  ctrlLabel: { fontSize: 10, fontWeight: "600", color: "rgba(255,255,255,0.75)", textAlign: "center" },
  ctrlLabelMuted: { color: Colors.statusDanger },
  ctrlBadge: {
  ...
  ctrlBadgeText: { fontSize: 9, fontWeight: "700", color: "#000" },
  ...
  handPanel: {
    ...
    zIndex: 30, paddingTop: spacing.space10,
  },
  handPanelHeader: {
    flexDirection: "row", alignItems: "center", justifyContent: "space-between",
  ...
  handPanelTitle: { flexDirection: "row", alignItems: "center", gap: spacing.space2 },
  handPanelTitleText: { ...typeScale.headingSM, color: "#FFF", fontWeight: "700", fontSize: 13 },
  handPanelEmpty: { alignItems: "center", paddingVertical: spacing.space6 },
  handPanelEmptyText: { ...typeScale.caption, color: "rgba(255,255,255,0.35)" },
  handPanelRow: {
    flexDirection: "row", alignItems: "center", gap: spacing.space2,
    paddingHorizontal: spacing.space4, paddingVertical: spacing.space3,
  ...
  handPanelAvatarText: { fontSize: 11, fontWeight: "700", color: Colors.palette.purple },
  handPanelName: { ...typeScale.bodySM, color: "rgba(255,255,255,0.80)", flex: 1, fontSize: 12 },
  promoteBtn: {
